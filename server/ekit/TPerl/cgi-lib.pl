...
# $Header: /au/apps/alltriton/cvs/TPerl/cgi-lib.pl,v 2.1 2004-09-30 11:42:57 triton Exp $
#
...
sub ReadParse 
  ...
  if (&MethGet) 
    ...
    $in = $ENV{'QUERY_STRING'};
  	...
  	{
    my $bytes = read(STDIN,$in,$ENV{'CONTENT_LENGTH'});
    die "Fatal error reading CGI data: read $bytes when expecting $ENV{'CONTENT_LENGTH'}\n" if ($bytes != $ENV{'CONTENT_LENGTH'});
  	}
